...
class SatelliteDataDownloader(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        csrf_token = response.xpath('//form[@id="loginForm"]/input[@type="hidden"][@name="csrf"]/@value').get()
    ...
    def parse_after_login(self, response):
        for i in self.download_urls:
            yield scrapy.Request(i[3], callback=self.get_download_links, meta={'displayId':i[0], 'Path':i[1], 'Row':i[2], 'catalog_id':i[4], 'entityId':i[5]})
    ...
    def get_download_links(self, response):
        ...
        count = len(response.xpath('//button[@class="btn btn-secondary downloadButton"]/@data-productid').getall())
        downloaded.setdefault(Path, {})
        downloaded[Path].setdefault(Row, {})
        downloaded[Path][Row] = count

        for i in response.xpath('//button[@class="btn btn-secondary downloadButton"]/@data-productid').getall():
            # response.meta['count'] = count
            yield scrapy.Request(f"https://earthexplorer.usgs.gov/download/{i}/{response.meta['entityId']}/{response.meta['catalog_id']}/", callback=self.download_data, meta=response.meta)
            # https://earthexplorer.usgs.gov/download/5e83d0b89619f7c4/LC80010022016230LGN01/EE/
